...
import {
  addChallengeLeadMutation,
  addTagToTagsetMutation,
  ...
  updateActorMutation,
  updateEcoverseMutation,
  updateOrganisationMutation,
  updateProfileMutation,
} from './graphql/mutations';
import { userQuery } from './graphql/queries/user';
import { Context } from './generated/graphql';
...
export class CherrytwistClient {
  ...
  public async testConnection(): Promise<Boolean> {
    ...
    if (!ecoverseName) {
      this.logger.error(
        `Unable to connect to ecoverse at location: ${this.config.graphqlEndpoint}`
      );
      return false;
    }
  ...
  async updateEcoverseContext(contex: Context): Promise<Boolean> {
    try {
      const result = await this.client.request(updateEcoverseMutation, contex);
      if (result) {
  ...
  async addTagToTagset(tagsetID: string, tagName: string): Promise<Boolean> {
    ...
    await this.client.request(addTagToTagsetMutation, addTagToTagsetVariable);
